...
typeof(lm.pred)
typeof(modeling[-train_rows, 'total_fires'])
postResample(lm.pred, modeling$total_fires[-train_rows])

```
```{r}
knn.fit <- train(total_fires ~ avg_temp, data = train,
                 method = 'knn',
                 preProcess = c('center', 'scale'),
                 trControl = trainControl('cv', number = 10),
...
boost.fit <- train(total_fires ~ avg_temp , data = train,
                 method = 'gbm',
...
topFive <- summary(boost.fit, plotit = F)
ggplot(head(topFive), aes(reorder(var, rel.inf), rel.inf)) +
